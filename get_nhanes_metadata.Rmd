...
get_all_variables <- function(tables) {
  details_folder = paste(output_folder, "variable_metadata/", sep="")
  ...
  for (nhanes_table in 1:nrow(tables)) {
    ...
    survey_year <- tables[nhanes_table, "Data.Year"]
...
```{r}
get_variables_in_table <- function(table_name, nhanes_data_group, year, response_details_folder) {
  all_variables <- data.frame(matrix(ncol = 5, nrow = 0))
  colnames(all_variables) <-c("Variable Name", "SAS Label", "English Text", "Target", "Table")
  ...
  if(length(table_variables) > 0) {
    for (variable in 1:nrow(table_variables)) {
      ...
      tryCatch({
        ...
        out_file <- paste(response_details_folder, year, "_", name, ".csv", sep="")
