...
get_tables_metadata <- function() {
  ...
  colnames(all_tables) <-c("Table", "Table Description", "Data Group", "Year")
  return(all_tables)
}
...
get_variables_metadata <- function(tables) {
  ...
  colnames(all_variables) <-c("Variable Name", "Table", "SAS Label", "English Text", "Target")
  ...
  colnames(all_codebooks) <-c("Variable Name", "Table", "Code or Value", "Value Description", 
    ...
    for (variable in 1:nrow(table_variables)) {
      ...
      if(variable_name != "SEQN") {
        tryCatch({
          ...
          variable_codebook <- cbind('Variable Name'=variable_name, variable_codebook)
